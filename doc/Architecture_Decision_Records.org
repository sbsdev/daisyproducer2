...
* DRAFT Upgrade dtbook2sbsform
- Deciders :: CE
- Date :: [2026-06-05 Fr]

~dtbook2sbsform~ converts DTBook XML to SBSForm, a markup language that
describes the layout of a braille document using macros. SBSForm already
contains the braille text translated by liblouis (in ASCII encoding);
~dtbook2sbsform~ invokes liblouis internally during the conversion. A new
version was deployed to the test server and needs validation before
promotion to production.

** Context and Problem Statement

Before promoting the new ~dtbook2sbsform~ to production, the braille output
of the new version must be validated against the current production output.
17 differences were found across 2 documents; the cause must be understood
and accepted by the customer before the upgrade can proceed.

** Validation run (2026-06-05)

The script ~scripts/compare-braille.bb~ was run against the 100 most recent
braille productions, generating braille with 10 parameter sets per document
(varying contraction grade, hyphenation, accent handling and other options).
...
The 7 document-level errors are unrelated to the upgrade:
- 4 × transient DB deadlock during upload to test server
- 2 × illustrated books (doc 13269, 13134) whose images are absent on the
  test server — version upload rejected with "File not found"
- 1 × doc 13255 ("rise up plus") has an empty ~author~ field that the
  stricter ~POST /api/documents~ validation correctly rejects

** Root cause investigation (2026-06-08)

*** Global whitelists: identical on both servers

The whitelist files that liblouis reads at translation time
(~/usr/local/share/liblouis/tables/sbs-de-g{1,2}-white.mod~ etc.) are
generated from the ~dictionary_globalword~ database table once a day at
21:00 UTC. A checksum comparison confirmed all four global whitelist files
are *byte-for-byte identical* on both servers (both regenerated last night).

The ~dictionary_globalword~ tables differ by 666 entries (all present in
prod, none in test), but these are unrelated to the 17 differing documents.
Global whitelists are not the cause.

*** Actual cause: local whitelist out of sync on production (2026-06-08)

The 17 differences are caused by a mismatch between the local word
database entries and the on-disk local whitelist files *on the production
server*:

- When braille was generated on *prod*, liblouis used the on-disk local
  whitelist files for docs 13165 and 13167.
- When the comparison script uploaded those documents to the *test* server
  and generated braille there, the application freshly exported the local
  words from the test database to disk (the 15-second wait in the script
  is for exactly this export). The test server therefore used up-to-date
  whitelists derived from the database.
- The prod on-disk local whitelists for these two documents are stale —
  they do not reflect the current state of ~dictionary_localword~ in the
  production database.
- The words that differ in the braille output (e.g. ~BIDELEEN~, ~K8MM9L~,
  ~VIELLE~) are defined in the stale prod whitelist files with their
  contracted forms (~BIDYEC~, ~K8X9L~, ~V0LLE~).

*~dtbook2sbsform~ itself behaves correctly.* The new version produces the
same output as the old version when both use whitelists generated from the
same database state. The upgrade is not the cause of any of the 17
differences.

The open question is why the prod local whitelist files are out of sync
with the database — see below.

*** Why are the local whitelists out of sync? (root cause found)

Application logs confirm the pattern for doc 13167:

- 2026-05-11 16:30–16:47 :: a translator confirmed local words; the
  whitelist was exported repeatedly as words were added
  (~Exporting local tables for 13167~)
- 2026-05-11 16:47:20 :: *last* whitelist export for doc 13167
- 2026-05-12 09:01:29 :: ~Deleted 15 local words that were not in unknown
  words for book 13167~ — but *no export was triggered*

The on-disk whitelist for doc 13167 has been stale since 2026-05-12,
retaining the 15 deleted words. Doc 13165's whitelist was last written
2026-04-28, predating the log retention window; same cause presumed.

The bug is in ~words/unknown.clj~: the ~update-words~ function calls
~delete-non-existing-unknown-words-from-local-words~ but never calls
~whitelists/export-local-tables~ afterwards. Every other word mutation
path (~put-word~ and ~delete-word~ in ~words/local.clj~) does trigger the
export. This path was simply overlooked.

*Fix committed in e786663:* ~update-words~ now calls
~whitelists/export-local-tables~ when it deletes one or more words.

** Verdict

~dtbook2sbsform~ behaves correctly. The upgrade is safe to promote to
production. The 17 apparent "differences" are an artefact of comparing
braille generated from a stale on-disk whitelist (prod, missing 15 deleted
local words for doc 13167) against braille generated from a fresh whitelist
export (test).

** Next steps

- [X] Confirm dtbook2sbsform is not the cause of the differences
- [X] Identify why local whitelists on prod are out of sync with the DB
- [X] Fix the missing export in ~update-words~ (commit e786663)
- [ ] Recover: regenerate stale local whitelists on prod for all affected
  documents (at minimum docs 13165 and 13167)
- [ ] Present findings to the customer and obtain sign-off for the upgrade
- [ ] Promote new ~dtbook2sbsform~ to production
- [ ] Sync 666 missing global word entries from prod DB to test DB
  (housekeeping — test server is a subset of prod)

** Links

- ~scripts/compare-braille.bb~ — the comparison script

* REJECTED Use German grade names in braille preview API
CLOSED: [2026-06-04 Mi]
- Deciders :: CE
- Date :: [2026-06-04 Mi]

The braille preview endpoint (~GET /api/documents/:id/preview/braille~)
accepts a ~contraction~ parameter with numeric values 0, 1, 2. The
official German names for these grades are Basisschrift (0), Vollschrift
(1), and Kurzschrift (2).

** Context and Problem Statement

Should the API parameter use the German terminology instead of the
numeric values, e.g. ~?grade=kurzschrift~ instead of ~?contraction=2~?

** Decision Outcome

Chosen option: *keep numeric values*, because:
- The numeric values are already established in the codebase and in use
  by clients (the braille comparison script, the UI)
- Renaming would be a breaking API change
- The mapping is documented in ~CLAUDE.md~

The parameter is currently named ~contraction~ which is slightly
misleading (it sounds like a boolean). A cleaner name would be ~grade~,
which matches the internal spec (~::grade~) — but again, a rename is a
breaking change not worth making now.

** Links

- [[https://bskdl.org/braillesysteme.html][BSKDL: Braillesysteme]] — official German grade terminology

* REJECTED Naming: ~author~ vs. ~creator~
CLOSED: [2026-06-02 Di 15:00]
- Deciders :: CE
- Date :: [2026-06-02 Di]

The person who wrote a book is called /creator/ in the Dublin Core
standard (~dc:Creator~), in the DTBook XML format, and in the Abacus
ERP XML. Inside the application the same concept is called /author/
(database column, Clojure keywords, REST API, UI label).

** Context and Problem Statement

Should the internal naming be unified with the external standard by
renaming ~author~ to ~creator~ throughout the application?

** Decision Drivers

- Avoid confusion :: for developers reading the code alongside the
  DTBook or Abacus schemas
- Avoid costly refactoring :: that brings no user-visible benefit
- Keep the REST API vocabulary natural :: for the domain ("author" is
  unambiguous to a library audience)

** Considered Options

- Keep ~author~ :: retain the current internal name; translate at the
  two external boundaries
- Rename to ~creator~ :: align the internal name with the Dublin Core /
  DTBook / Abacus terminology throughout

** Decision Outcome

Chosen option: *Keep ~author~*, because the internal naming is already
perfectly consistent (DB column, SQL queries, Clojure/ClojureScript
code, i18n label, REST API all use ~author~). The translation to
~creator~ already happens at exactly the right places:
- Inbound (Abacus XML → application) :: [[file:~/src/dp2/src/clj/daisyproducer2/documents/abacus.clj:::author \[:MetaData :dc :creator text\]][abacus.clj:77]]
  #+begin_src clojure
    (def ^:private param-mapping
      { ...
       ...
       ... })
  #+end_src
- Outbound (application → DTBook XML) :: [[file:~/src/dp2/src/clj/daisyproducer2/documents/update_metadata.clj::{:author "dc:Creator"][update_metadata.clj:9]]
  #+begin_src clojure
    (def mapping
      {:author "dc:Creator" ...})
  #+end_src

Renaming would require a DB migration, changes to all SQL, Clojure,
ClojureScript, and i18n files, with no functional or user-visible
benefit.

*** Positive Consequences

- No change required; existing code stays consistent

*** Negative Consequences

- Developers need to know that ~author~ in the application corresponds
  to ~dc:Creator~ in the DTBook/Abacus world (hence this ADR)

** Links

- ~src/clj/daisyproducer2/documents/abacus.clj~ — inbound mapping
- ~src/clj/daisyproducer2/documents/update_metadata.clj~ — outbound mapping
- [[https://www.dublincore.org/specifications/dublin-core/dcmi-terms/#creator][Dublin Core: dc:Creator]]
