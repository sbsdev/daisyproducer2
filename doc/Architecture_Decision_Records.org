...
*** Why are the local whitelists out of sync?

The application exports local whitelist files asynchronously: changes to
~dictionary_localword~ are queued and written to disk within seconds
(~whitelists/async.clj~). A mismatch can occur if:

1. Words were modified directly in the database, bypassing the application
2. The export failed silently (exception caught in the async worker)
3. The application was restarted while exports were queued but not yet
   written
4. The whitelist file on disk was restored from an older backup

This needs to be investigated and a recovery mechanism considered.
...
fresh whitelist exports (test) against stale on-disk whitelists (prod).

The stale local whitelists on the production server are a separate bug that
exists independently of this upgrade and must be addressed.

** Next steps

- [X] Confirm dtbook2sbsform is not the cause of the differences
- [ ] Investigate why local whitelists on prod are out of sync with the DB
- [ ] Recover: regenerate all stale local whitelists on prod
