...
<xsl:choose>
   ...
   <!--PATTERN sbs_class_implicit-->


	<!--RULE -->
<xsl:template match="dtb:h1[@class='implicit']" priority="1000" mode="M4">

		<!--ASSERT -->
<xsl:choose>
         <xsl:when test="not(ancestor::dtb:bodymatter)"/>
         <xsl:otherwise>
            <xsl:message>The class='implicit' is not allowed on h1 elements within bodymatter.</xsl:message>
         </xsl:otherwise>
      </xsl:choose>
      <xsl:apply-templates select="*" mode="M4"/>
   </xsl:template>
...
<xsl:choose>
   ...
   <!--PATTERN sbs_continuation-->
	...
	<!--RULE -->
<xsl:template match="*[@brl:continuation]" priority="1000" mode="M7">
