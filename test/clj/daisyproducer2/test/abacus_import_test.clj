...
(defrecord Raw [product-number title creator language source date source-publisher source-edition production-series-number reihe aufwand daisy-producer])
...
(defn- xml-sample
  [{:keys [product-number title creator source language date source-publisher source-edition production-series-number reihe aufwand daisy-producer]}]
  [:AbaConnectContainer
   [:Task
    [:Transaction
     [:DocumentData
